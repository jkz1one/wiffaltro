...
func build(lab: PitchBatLab) -> void:
	...
	for index in range(9):
		...
		button.custom_minimum_size = Vector2(151.0, 42.0)
...
func refresh() -> void:
	...
	_title.text = "PITCHES  •  " + pitcher.definition.display_name
	var fatigue: float = maxf(pitcher.fatigue_ratio(), _lab._fatigue)
	_condition.text = "%d thrown  •  %.0f%% stamina  •  %s" % [
		pitcher.pitch_count, pitcher.stamina_percent() * 100.0,
		PitchExecutionModel.fatigue_stage_name(fatigue)
	]
