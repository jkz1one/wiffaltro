...
static func begin_swing(
	...
	if lab._match_mode:
		var batter_definition: PlayerDefinition = (
			lab._match_state.batter().definition
		)
...
static func ensure_miss(lab: PitchBatLab) -> ContactResult:
	...
	if lab._swing_tracker == null or lab._pitch_actor == null:
		return null
	var result: ContactResult = lab._swing_tracker.force_miss(
		lab._pitch_actor.state
	)
	if result != null:
		_register_miss(lab, result)
...
static func _register_miss(
	...
	PitchBatLabFeelSupport.note_swing(
		lab,
		profile.id,
