...
static func step(
	...
	var acceleration_start := PitchAerodynamics.acceleration(
		state.velocity,
		state.orientation,
	...
	var midpoint_velocity := (
		state.velocity
		+ acceleration_start * delta_seconds * 0.5
	)
	var midpoint_orientation := PitchAerodynamics.advance_orientation(
		state.orientation,
		state.angular_velocity,
		delta_seconds * 0.5
	)

	var acceleration_midpoint := PitchAerodynamics.acceleration(
		midpoint_velocity,
