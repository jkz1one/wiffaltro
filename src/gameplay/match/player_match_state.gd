...
func batting_hand() -> int:
	return (
		batting_hand_override
		if definition.switch_hitter and batting_hand_override >= 0
		else definition.bats
	)


func bats_left() -> bool:
	return batting_hand() == PlayerDefinition.Handedness.LEFT
...
static func create(player_definition: PlayerDefinition) -> PlayerMatchState:
	...
	result.stamina_max = (
		(BASE_STAMINA + float(player_definition.stamina) * STAMINA_PER_RATING) * 1.08
	)
