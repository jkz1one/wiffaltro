...
const FOUL_MIN_SPRAY_DEGREES: float = 48.0
const FOUL_MAX_SPRAY_DEGREES: float = 70.0
...
static func resolve_swept_segment(
	...
	var segment_seconds: float = maxf(0.000001, current_pitch_time - previous_pitch_time)
	var start_alpha: float = clampf(
		(overlap_start - previous_pitch_time) / segment_seconds, 0.0, 1.0
	)
	var end_alpha: float = clampf((overlap_end - previous_pitch_time) / segment_seconds, 0.0, 1.0)
	var ball_start: Vector3 = previous_position.lerp(pitch_state.position, start_alpha)
	var ball_end: Vector3 = previous_position.lerp(pitch_state.position, end_alpha)
	var swing_start: float = overlap_start - intent.start_time_seconds
	var swing_end: float = overlap_end - intent.start_time_seconds
	var center_start: Vector3 = swing_center_position(swing_start, intent, profile)
	var center_end: Vector3 = swing_center_position(swing_end, intent, profile)
	...
	var relative_start_z: float = (ball_start.z - center_start.z) / BARREL_DEPTH_RADIUS_M
	var relative_end_z: float = (ball_end.z - center_end.z) / BARREL_DEPTH_RADIUS_M
	var relative_delta_z: float = relative_end_z - relative_start_z
	var closest_alpha: float = 0.0
	if absf(relative_delta_z) > 0.000001:
		closest_alpha = clampf(-relative_start_z / relative_delta_z, 0.0, 1.0)
	var closest_relative_z: float = lerpf(relative_start_z, relative_end_z, closest_alpha)
	if absf(closest_relative_z) > 1.0:
		return null

	var contact_position: Vector3 = ball_start.lerp(ball_end, closest_alpha)
	var contact_center: Vector3 = center_start.lerp(center_end, closest_alpha)
	return _resolve_at_contact(
		pitch_state, contact_position, contact_center, intent, profile, contact_rating, power_rating
	)
...
static func timing_miss(
	...
	var swing_elapsed: float = pitch_state.elapsed_time - intent.start_time_seconds
	var contact_center: Vector3 = swing_center_position(swing_elapsed, intent, profile)
	...
	var nx: float = result.horizontal_error_m / (profile.contact_radius_x_m * contact_factor)
	var ny: float = result.vertical_error_m / (profile.contact_radius_y_m * contact_factor)
	result.spatial_quality = clampf(1.0 - sqrt(nx * nx + ny * ny), 0.0, 1.0)
	result.miss_reason = (
...
static func _resolve_at_contact(
	...
	var contact_factor: float = _contact_factor(contact_rating)
	var power_factor: float = lerpf(0.85, 1.15, clampf(float(power_rating) / 10.0, 0.0, 1.0))

	var horizontal_error: float = contact_position.x - contact_center.x
	var vertical_error: float = contact_position.y - contact_center.y
	...
	var nx: float = horizontal_error / (profile.contact_radius_x_m * contact_factor)
	var ny: float = vertical_error / (profile.contact_radius_y_m * contact_factor)
	...
	var quality: float = clampf(1.0 - sqrt(normalized_error_squared), 0.0, 1.0)
	...
	var timing_ratio: float = clampf(depth_error / profile.contact_depth_m, -1.0, 1.0)
	var handedness_sign: float = -1.0 if not intent.handedness_left else 1.0
	result.spray_degrees = (timing_ratio * MAX_SPRAY_DEGREES * handedness_sign)
	if result.outcome == ContactResult.Outcome.FOUL:
		var foul_side: float = signf(result.spray_degrees)
		if is_zero_approx(foul_side):
			foul_side = signf(horizontal_error)
		if is_zero_approx(foul_side):
			foul_side = handedness_sign
		var foul_severity: float = clampf(
			1.0 - quality / maxf(0.001, profile.minimum_contact_quality), 0.0, 1.0
		)
		result.spray_degrees = (
			foul_side * lerpf(FOUL_MIN_SPRAY_DEGREES, FOUL_MAX_SPRAY_DEGREES, foul_severity)
		)

	var vertical_ratio: float = clampf(
		vertical_error / (profile.contact_radius_y_m * contact_factor), -1.0, 1.0
	)
	result.launch_angle_degrees = clampf(
		profile.attack_angle_degrees + vertical_ratio * 32.0, -22.0, 55.0
	)

	var incoming_speed: float = pitch_state.velocity.length()
	var ideal_exit_speed: float = (
		(profile.bat_speed_mps + incoming_speed * BASE_TRANSFER_FACTOR)
...
static func swing_center_position(
	swing_elapsed_seconds: float, intent: SwingIntent, profile: SwingProfileDefinition
) -> Vector3:
	...
	return Vector3(
		intent.aim_point.x,
		intent.aim_point.y + (center_z - CONTACT_PLANE_Z) * tan(attack_radians),
		center_z
	)


static func _swing_center_z(swing_elapsed_seconds: float, profile: SwingProfileDefinition) -> float:
	if swing_elapsed_seconds <= profile.sweet_spot_seconds:
		...
		return CONTACT_PLANE_Z + lerpf(-profile.contact_depth_m * 0.5, 0.0, load_progress)
	var finish_progress: float = clampf(
		inverse_lerp(
			profile.sweet_spot_seconds, profile.contact_window_end_seconds, swing_elapsed_seconds
		),
		0.0,
		1.0
	)
	return CONTACT_PLANE_Z + lerpf(0.0, profile.contact_depth_m * 0.5, finish_progress)


static func _contact_factor(contact_rating: int) -> float:
	return lerpf(0.82, 1.18, clampf(float(contact_rating) / 10.0, 0.0, 1.0))


static func _primary_miss_reason(
	normalized_x: float, normalized_y: float, normalized_timing: float
