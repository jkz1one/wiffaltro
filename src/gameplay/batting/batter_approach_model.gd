...
func trigger_z(
	...
	var speed_pressure: float = clampf(
		inverse_lerp(12.0, 32.0, plate_speed_mps),
		0.0,
		1.0
	)
...
func decide(
	...
	var body_side: float = (
		-1.0
		if batter.bats == PlayerDefinition.Handedness.LEFT
		else 1.0
	)
	var inside_amount: float = ball_xy.x * body_side
	var outside_distance: float = maxf(0.0, absf(ball_xy.x) - ZONE_HALF_WIDTH_M)
	var vertical_distance: float = maxf(
		0.0,
		absf(ball_xy.y - ZONE_CENTER_Y_M) - ZONE_HALF_HEIGHT_M
	)
	var chase_distance: float = Vector2(
		outside_distance,
		vertical_distance
	).length()
	...
	var center_score: float = clampf(
		1.0 - Vector2(
			ball_xy.x / 0.52,
			(ball_xy.y - ZONE_CENTER_Y_M) / 0.58
		).length(),
		0.0,
		1.0
	)
	var outside_sweet_spot: float = clampf(
		1.0 - absf(inside_amount + 0.14) / 0.38,
		0.0,
		1.0
	)
	var inside_penalty: float = clampf(
		inverse_lerp(0.05, ZONE_HALF_WIDTH_M, inside_amount),
		0.0,
		1.0
	) * 0.14
	var recognition_load: float = maxf(
		0.0,
		pitch.recognition_difficulty - awareness * 0.90
	)
	var speed_load: float = clampf(
		inverse_lerp(14.0, 31.0, plate_speed_mps),
		0.0,
		1.0
	)
	...
	aim_sigma += speed_load * pitch.timing_difficulty * 0.035
	aim_sigma += chase_distance * 0.22
	aim_sigma += inside_penalty * 0.22
	aim_sigma -= awareness * 0.095
	...
	var aim: Vector2 = ball_xy + Vector2(
		rng.randfn(0.0, aim_sigma),
		rng.randfn(0.0, aim_sigma)
	)
	var punish_score: float = clampf(
		center_score * pitch.mistake_punish + awareness * 0.75,
		0.0,
		1.5
	)
	var power_chance: float = clampf(
		0.12
		+ float(batter.power - batter.contact) * 0.045
		+ punish_score * 0.34,
		0.05,
		0.78
	)
